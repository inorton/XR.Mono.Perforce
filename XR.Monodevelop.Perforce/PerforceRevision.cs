...
namespace XR.Monodevelop.Perforce
{
    ...
    {
        public PerforceRevision( PerforceRepo repo, DateTime time, string author, string msg ) : base( repo, time, author, msg )
        {
        }

        public PerforceRevision( PerforceRepo repo ) : base ( repo )
        {
        }
        ...
        {
            if ( P4Rev > 1 )
                return P4Repo.GetHistory( FilePath.FullPath, new PerforceRevision( P4Repo ) { P4Rev = P4Rev - 1 } ).FirstOrDefault();
            return null;
        }
